...
func (rm *rootModule) findCoreSchema() error {
	...
	wasDecoderReady := rm.IsDecoderReady()
...
func (rm *rootModule) DecodeFiles() (hcl.Diagnostics, error) {
	...
	if err != nil {
		diags = append(diags, &hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("Module directory %s does not exist or cannot be read.", rm.Path()),
		})
		return diags, fmt.Errorf("failed to read module at %q: %w", rm.Path(), err)
	}
	...
	for _, info := range infos {
		...
		if err != nil {
			return diags, fmt.Errorf("failed to read %q: %s", name, err)
		}
	...
	if rm.IsProviderSchemaLoaded() {
		s, mDiags := tfschema.MergeCoreWithJsonProviderSchemas(files, bodySchema, rm.providerSchema)
	...
	if rm.decoder == nil {
		...
		if err != nil {
			return diags, fmt.Errorf("failed to create new decoder: %w", err)
		}
		rm.decoder = d
	} else {
		rm.decoder.ResetSchema(bodySchema)
	}

	for name, f := range files {
		err := rm.decoder.LoadFile(f, name)
		if err != nil {
			return diags, fmt.Errorf("failed to load a file: %w", err)
		}
	}
